...
def objective(trial, data_list, data_cf_list, sens, sens_cf):
    ...
    # # theta = trial.suggest_float("theta", 1e-5, 1, log=True)
    ...
    # num_layers = trial.suggest_categorical("num_layers", [2, 3, 4, 5])
    # hid_dim = trial.suggest_categorical("hid_dim", [32, 64, 128, 256])  
    # w1 = trial.suggest_float("w1", 1e-5, 1, log=True)
    ...
    # # gaan
    # 奇怪，gaan会报cross_entropy no in 0,1错，RuntimeError: all elements of target should be between 0 and 1
    # lr = trial.suggest_float("lr", 1e-5, 1e-1, log=True)
    # dropout = trial.suggest_float("dropout", 1e-6, 1, log=True)
    # weight_decay = trial.suggest_float("weight_decay", 1e-6, 1, log=True)
    # num_layers = trial.suggest_categorical("num_layers", [3, 4, 5])
    # hid_dim = trial.suggest_categorical("hid_dim", [32, 64, 128, 256]) 
    # noise_dim = trial.suggest_categorical("noise_dim", [4, 8, 16, 32, 64]) 
    ...
    # sample_size = trial.suggest_int("sample_size", 1, 10)
    ...
    weight_decay = trial.suggest_float("weight_decay", 1e-6, 1, log=True)
    num_layers = trial.suggest_categorical("num_layers", [2,3, 4, 5])
    hid_a = trial.suggest_categorical("hid_a", [32, 64, 128, 256]) 
    hid_s = trial.suggest_categorical("hid_s", [2, 3, 4, 6, 8])  
    alpha = trial.suggest_float("alpha", 1e-5, 1, log=True)
    ...
    def train_and_predict(data, contamination=0.05, threshold_percentile=95):
        ...
        # model = ANOMALOUS(lr=lr, gamma=gamma, weight_decay=weight_decay, contamination=contamination)
        # model = AnomalyDAE(lr=lr, dropout=dropout, weight_decay=weight_decay, num_layers=num_layers, hid_dim=hid_dim, emb_dim=emb_dim, contamination=contamination) #  alpha=alpha, theta=theta, eta=eta,
        # model = CoLA(lr=lr, dropout=dropout, weight_decay=weight_decay, num_layers=num_layers, hid_dim=hid_dim, contamination=contamination)
        # model = CONAD(lr=lr, dropout=dropout, weight_decay=weight_decay, num_layers=num_layers, hid_dim=hid_dim, contamination=contamination)        
        # model = DMGD(lr=lr, dropout=dropout, weight_decay=weight_decay, num_layers=num_layers, hid_dim=hid_dim, contamination=contamination)
        # model = DOMINANT(lr=lr, dropout=dropout, weight_decay=weight_decay, num_layers=num_layers, hid_dim=hid_dim, contamination=contamination)
        # model = DONE(lr=lr, dropout=dropout, weight_decay=weight_decay, num_layers=num_layers, hid_dim=hid_dim, w1=w1, w2=w2, w3=w3, w4=w4, w5=w5, contamination=contamination)      
        # model = GAAN(lr=lr, dropout=dropout, weight_decay=weight_decay, num_layers=num_layers, hid_dim=hid_dim, noise_dim=noise_dim, contamination=contamination)  
        # model = GADNR(hid_dim=hid_dim,num_layers=num_layers,sample_size=sample_size,sample_time=sample_time,lambda_loss1=lambda_loss1,lambda_loss2=lambda_loss2,lambda_loss3=lambda_loss3,lr=lr,dropout=dropout,weight_decay=weight_decay)
        # model = GAE(hid_dim=hid_dim,num_layers=num_layers,lr=lr,dropout=dropout,weight_decay=weight_decay, contamination=contamination)
        model = GUIDE(hid_a=hid_a, hid_s=hid_s, num_layers=num_layers, lr=lr, dropout=dropout, weight_decay=weight_decay, alpha=alpha, contamination=contamination) 
        model.fit(data)
    ...
    for outlier_type in outlier_types:
        ...
        y_score_cf, y_pred_cf, threshold_cf = train_and_predict(data_cf)
        ...
        auc += (eval_result[outlier_type]['auc'])/6
    return auc
