...
def gen_structural_outliers_cf(data, data_cf, m, n, p=0.2, y_outlier=None, random_state=None):
    ...
    if y_outlier is not None:
        node_set = set(list(np.where(y_outlier==0)[0])) # 如果提供了y_outlier标签，则从中选择非异常节点作为候选节点集
    else:   
        y_outlier = np.zeros(data.num_nodes) # 如果未提供y_outlier，则初始化为全0数组，并将所有节点作为候选节点集
        node_set = set(range(data.num_nodes))   
        
    r = np.random.RandomState(random_state)
    
    outlier_idx = r.choice(list(node_set), size=int(m * n), replace=False) # 随机选择m * n个节点作为异常节点
...
def gen_contextual_outliers_cf(data, data_cf, n, k, random_state=None):
    node_set = set(range(data.num_nodes))
    r = np.random.RandomState(random_state)
    outlier_idx = r.choice(list(node_set), size=n, replace=False) # 随机选择n个节点作为异常节点
    candidate_set = node_set.difference(set(outlier_idx)) # 确定候选节点集
    candidate_idx = r.choice(list(candidate_set), size=int(n * k)) # 从候选节点集中随机选择n * k个候选节点
    ...
    def apply_contextual_outliers(data_, outlier_idx, candidate_idx, k):
        for i, idx in enumerate(outlier_idx):
            cur_candidates = candidate_idx[k * i: k * (i + 1)] # 当前异常节点对应的候选节点
            euclidean_dist = torch.cdist(data_.x[idx].unsqueeze(0), data_.x[list(cur_candidates)]) # 计算当前异常节点与候选节点之间的欧氏距离
            max_dist_idx = torch.argmax(euclidean_dist) # 找到距离最远的节点index
            max_dist_node = list(cur_candidates)[max_dist_idx]
            with torch.no_grad():
                data_.x[idx] = data_.x[max_dist_node] # 用距离最远节点的x替换当前异常节点的x
        return data_
...
def gen_dice_outliers_cf(data, data_cf, n, r_perturb, y_outlier=None, random_state=None):
    ...
    edge_index_cf = data_cf.edge_index
    labels = torch.from_numpy(data.y.reshape(-1,))
    labels_cf = torch.from_numpy(data_cf.y.reshape(-1,))
    
    undirected_edge_index = to_undirected(edge_index)
    undirected_edge_index_cf = to_undirected(edge_index_cf)
    is_symmetric = torch.equal(undirected_edge_index, undirected_edge_index.t())
    is_symmetric_cf = torch.equal(undirected_edge_index_cf, undirected_edge_index_cf.t())
    ...
    def apply_dice_outliers(data, outlier_idx, r_perturb, node_set, labels, is_symmetric):
        ...
        for node1 in outlier_idx: # 遍历每个anomaly nodes
            node_drop_list = edge_index[:, edge_index[0] == node1][1] # 获取当前异常节点的Neighbour list
            no_edge = (node_set - set(node_drop_list.tolist())) # Non-neighbour node list (candidate nodes)
            label_diff = torch.nonzero(labels != labels[node1]).squeeze() # Nodes whose label different with node i
            node_add_list = torch.tensor(list(no_edge & set(label_diff.tolist()))) # Nodes: Both not neighbour and diff label

            drop_num = int(np.ceil(r_perturb * np.min((len(node_drop_list), data.num_nodes - len(node_drop_list))))) # Calculate edge to drop
    ...
    data_cf = apply_dice_outliers(data_cf, outlier_idx, r_perturb, node_set, labels_cf, is_symmetric_cf)
...
def gen_path_outliers_cf(data, data_cf, n, k, random_state=None):
    ...
    graph = graph.to_undirected() # 转化为无向图
        
    y_outlier = np.zeros(data.num_nodes)
    r = np.random.RandomState(random_state)
    
    def apply_path_outliers(data, outlier_idx, candidate_idx, k):
        for i, idx in enumerate(outlier_idx):  # calculate the shortest path for each candidate
            cur_candidates = candidate_idx[k * i: k * (i + 1)]
            path = []
            path_len = []
            for can in cur_candidates:
                try:
                    shortest_path = nx.shortest_path(graph, source=idx, target=can) # shortest path
            ...
            if np.sum(np.array(path_len) > 0) > (k // 2):
                max_path_idx = np.random.choice(np.where(path_len == np.max(path_len))[0]) # 在最短路最长的里面随机选一个
